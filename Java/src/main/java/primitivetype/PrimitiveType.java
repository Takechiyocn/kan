...
public class PrimitiveType {
     ...
     * 最开始Java中char类型用16位无符号整数来描述unicode中最早一批收录的字符(BMP:Basic Multilingual Plane)，即utf-16前身USE-2，
...
//        char charEmoji = '\uD83D\uDE42';
        ...
        // 4 4
        System.out.println("字符串长度：" + name.length() + ", 编码占用字节数：" + name.getBytes(StandardCharsets.UTF_8).length);
        // 2 6
        System.out.println("字符串长度：" + name1.length() + ", 编码占用字节数：" + name1.getBytes(StandardCharsets.UTF_8).length);
        // 6 10
        System.out.println("字符串长度：" + name2.length() + ", 编码占用字节数：" + name2.getBytes(StandardCharsets.UTF_8).length);
