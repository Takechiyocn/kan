 ...
 *  分类：
 *    a. Oracle推荐使用
 *      1. 普通内部类(推荐？) {@link InnerClassNormal}
 *      2. 静态内部类：又称静态嵌套类 {@link InnerClassStatic}
 *    b. Oracle不推荐使用(非静态内部类) {@link InnerClassStatic}
 *      1. 局部内部类(定义在方法体内)：又称局部类 {@link InnerClassLocal}
 *      2. 匿名内部类(定义在初始化赋值)：
 *         a. 常规匿名内部类 {@link InnerClassAnonymous}
 ...
 *         b. lambda匿名内部类 {@link InnerClassAnonymousLambda}
 *            //  此处实现可视为一个FunctionalInterface对象
 *            (抽象方法参数xxx) -> {
 *               // 抽象方法实现
 *            }
