    ...
    public static void eReflection() throws ClassNotFoundException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        for (Constructor con : constructors) {
            ...
            if (parameterArray.length == 1) {
                if (Constants.STRING_PARAMETER.equals(parameterArray[0])) {
                    // Employee(String)
            ...
            if (parameterArray.length == 2) {
                if (Constants.STRING_PARAMETER.equals(parameterArray[0]) &&
                        Constants.DOUBLE_PARAMETER.equals(parameterArray[1])) {
            ...
            if (parameterArray.length == 5) {
                if (Constants.STRING_PARAMETER.equals(parameterArray[0]) &&
                        Constants.DOUBLE_PARAMETER.equals(parameterArray[1]) &&
                        Constants.INTEGER_PARAMETER.equals(parameterArray[2]) &&
                        Constants.INTEGER_PARAMETER.equals(parameterArray[3]) &&
                        Constants.INTEGER_PARAMETER.equals(parameterArray[4])) {
